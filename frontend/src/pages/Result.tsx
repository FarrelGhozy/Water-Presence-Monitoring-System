...
function SatelliteBreakdownInner({ data }: { data: SatelliteData }) {
  const sources = [
    { name: 'Sentinel-1 SAR', icon: 'satellite', data: [`Water: ${data.sar.waterPercentage ?? 'N/A'}%`, `Backscatter: ${data.sar.backscatterMean ?? 'N/A'} dB`], badge: data.sar.confidence === 'no_data' ? 'warning' as const : data.sar.confidence === 'high' ? 'success' as const : 'default' as const, badgeLabel: data.sar.confidence },
    { name: 'Sentinel-2 NDWI', icon: 'globe', data: [`NDWI: ${data.ndwi.value ?? 'N/A'}`, data.ndwi.cloudCover !== null ? `Cloud: ${data.ndwi.cloudCover}%` : 'Tidak tersedia'], badge: data.ndwi.available ? 'success' as const : 'warning' as const, badgeLabel: data.ndwi.available ? 'Tersedia' : 'Tidak Tersedia' },
    { name: 'CHIRPS Rainfall', icon: 'rain', data: [`7-hari: ${data.chirps.rainfall7day_mm} mm`, `Trend: ${data.chirps.trend}`], badge: 'default' as const, badgeLabel: data.chirps.trend },
    { name: 'Soil Type', icon: 'soil', data: [`Tipe: ${data.soil.type}`], badge: 'default' as const, badgeLabel: data.soil.type },
    { name: 'Elevation (SRTM)', icon: 'mountain', data: [`Elevasi: ${data.elevation.meters} m`, `Terrain: ${data.elevation.terrain}`], badge: 'default' as const, badgeLabel: data.elevation.terrain },
  ]
        ...
        <Card key={s.name}>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <Icon name={s.icon as any} className="text-brand-400" />
...
export default function Result() {
  ...
  if (obsError || !obsData) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        <Icon name="search" className="text-4xl mb-4 block text-gray-400" />
  ...
  if (obsData.observation.status === 'error') {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        <Icon name="warning" className="text-4xl mb-4 block text-red-400" />
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <>
          ...
          <Card className="mb-6">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-gray-300 flex items-center gap-2"><Icon name="lightbulb" className="text-brand-400" /> Analisis AI</h3>
                ...
                <ul className="space-y-1">
                  {analysis.recommendations.map((r, i) => (
                    <li key={i} className="text-sm text-gray-400 flex items-start gap-2">
                      <Icon name="check" className="text-brand-400 mt-0.5" />
                      {r}
                    </li>
                  ))}
                </ul>
      ...
      {satellite && (
        <div className="mb-6">
          <h3 className="text-sm font-semibold text-gray-300 mb-3 flex items-center gap-2"><Icon name="satellite" className="text-brand-400" /> Data Satelit</h3>
