...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  \textbf{Nama Anggota 2} & : \textit{(Zia Zarin)} \\[0.3cm]
  \textbf{Nama Anggota 3} & : \textit{(Rasya Naufal)} \\[0.3cm]
  \hline
\end{tabularx}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item Panel 2: AI Analysis summary — confidence gauge 0-100\%, natural language assessment dari Gemini, water presence verdict, risk indicators
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  & - \texttt{observations}: userId, lokasi (lat/lng), photoUrl, timestamp, status (pending/processing/completed/error) \\[0.2cm]
...
\begin{tabularx}{\textwidth}{|l|l|X|}
  ...
  \hline
  POST & /api/v1/observations & Submit observasi (foto + GPS) \\
...
\begin{enumerate}
  \item \textbf{Multi-Source Satellite Analysis (Bukan Sekadar Foto)}
  \begin{itemize}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Metadata: timestamp, lokasi (provinsi/kabupaten), foto user (sebagai referensi)
    \end{itemize}
...
\begin{enumerate}
  ...
  \item Backend validates input (location range, photo size $<$ 5MB, file type JPEG/PNG)
...
\begin{itemize}
  \item \textbf{Expected Accuracy}: F1-score 0.75-0.90 (lebih tinggi dari pendekatan photo-based karena pakai SAR multi-sumber)
